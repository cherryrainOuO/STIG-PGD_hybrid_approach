...
import dif_model
from dif_utils import calc_even_size, produce_spectrum
...
class TrainerMultiple(nn.Module):
    def __init__(self, hyperparams):
        ...
        self.unet = dif_model.Unet(self.device, self.ch_i, self.ch_o, self.arch,
...
class TrainerSingle(nn.Module):
    def __init__(self, hyperparams):
        ...
        # Model initialization
        self.AE = dif_model.Unet(self.device, self.ch_i, self.ch_o, self.arch,
                             activ='leak', depth=self.depth, concat=self.concat).to(self.device)
