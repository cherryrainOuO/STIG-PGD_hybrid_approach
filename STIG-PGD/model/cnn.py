...
def build_classifier_from_vit(opt) :
...
def eval_from_opt(opt) :
    ...
    eval_accs, eval_losses, eval_precision, eval_recall, eval_f1= evaluation(model, test_loaders, opt)

    for (name, acc, loss, pre, re, f1) in zip(eval_roots, eval_accs, eval_losses, eval_precision, eval_recall, eval_f1) :
        print('[Profile of {}]'.format(name))
        logs.append('[Profile of {}]'.format(name))
        print('accuracy & loss : {:.4f}% | {:.4f}\n'.format(acc, loss))
        print('precision & recall & F1 : {:.4f}% | {:.4f}% | {:.12f}%\n'.format(pre, re, f1))
        logs.append('accuracy & loss : {:.4f}% | {:.4f}\n'.format(acc, loss))
        logs.append('precision & recall & F1 : {:.4f}% | {:.4f}% | {:.12f}%\n'.format(pre, re, f1))
...
def evaluation(model, dataloaders, opt):
    ...
    results_precision, results_recall, results_f1 = [], [], []
    ...
    return results_acc, results_loss, results_precision, results_recall, results_f1
